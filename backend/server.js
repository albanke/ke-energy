...
// -------------------- Static front-end (opzionale) --------------------
// Se vuoi far girare tutto (front+api) in locale: imposta SERVE_STATIC=true
...
if (SERVE_STATIC) {
  ...
  app.use((req, res) => {
    if (req.path.endsWith(".pdf")) return res.status(404).send("PDF non trovato");
