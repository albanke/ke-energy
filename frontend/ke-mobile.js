...
(function () {
  ...
  function initHamburger() {
    ...
    // Leggi i link dalla navbar originale
    ...
    if (origLinks) {
      origLinks.querySelectorAll('a').forEach(function (a) {
        ...
        linksHTML += '<a href="' + a.getAttribute('href') + '"' +
          (isCta ? ' class="is-cta"' : '') + '>' +
    ...
    drawer.innerHTML =
      '<div class="ke-nav-drawer__header">' +
        '<a class="ke-nav-drawer__logo" href="index.html">' +
          ...
          '<span>KE Energy</span>' +
        '</a>' +
        '<button class="ke-nav-drawer__close" aria-label="Chiudi menu">✕</button>' +
      '</div>' +
    ...
    // Mostra/nascondi solo su mobile
    function checkSize() {
  ...
  /* ══════════════════════════════════════════════════════
     FLOATING CTA (WhatsApp + Preventivo) — solo mobile
  ══════════════════════════════════════════════════════ */
  function initFAB() {
    if (document.querySelector('.ke-fab-group')) return;
    if (window.innerWidth > 768) return;

    // Non mostrare sulla pagina login
    if (document.querySelector('.login-screen, .page-login')) return;
    ...
    group.innerHTML =
      '<a class="ke-fab ke-fab--wa" href="https://wa.me/393408043834" target="_blank" rel="noopener" aria-label="Contattaci su WhatsApp">' +
        ...
        'WhatsApp' +
      '</a>' +
      '<a class="ke-fab ke-fab--cta" href="preventivo.html" aria-label="Richiedi preventivo">' +
        'Preventivo →' +
      '</a>';

    document.body.appendChild(group);

    // Nascondi FAB quando si scrolla verso il basso, mostralo quando si scrolla su
    var lastScroll = 0;
    var ticking = false;
  ...
  /* ══════════════════════════════════════════════════════
     NAV: scroll effect — sfondo solido dopo 60px
  ══════════════════════════════════════════════════════ */
  ...
  /* ══════════════════════════════════════════════════════
     SMOOTH TOUCH SCROLL per hscroll su mobile
  ══════════════════════════════════════════════════════ */
  ...
  /* ══════════════════════════════════════════════════════
     CONTACT: aggiungi icone ai link tel/wa su mobile
  ══════════════════════════════════════════════════════ */
